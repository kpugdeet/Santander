...
def generate_embeddings():
    # Import data
    mnist = input_data.read_data_sets('./mnist', one_hot=True, fake_data=False)
    ...
    writer = tf.summary.FileWriter('./logs' + '/projector', sess.graph)
    ...
    embed.metadata_path = os.path.join('./logs' + '/projector/metadata.tsv')
    ...
    saver.save(sess, os.path.join('./logs', 'projector/a_model.ckpt'), global_step=10000)
...
def generate_metadata_file():
    ...
    save_metadata('./logs' + '/projector/metadata.tsv')

if tf.gfile.Exists('./logs' + '/projector'):
    tf.gfile.DeleteRecursively('./logs' + '/projector')
    tf.gfile.MkDir('./logs' + '/projector')
tf.gfile.MakeDirs('./logs'  + '/projector') # fix the directory to be created
